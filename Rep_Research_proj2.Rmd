---
title: "Impacts of US Storms on Health and Economy"
...
## 1: Synopsis
The objective of the assignment is to explore the NOAA Storm Database and explore how the severe weather events affect both population and economy.
Time period of the data is between 1950 and November 2011. 
In the earlier years of the database there are generally fewer events recorded, most likely due to a lack of good records. More recent years are generally more complete.

The following analysis helps to identify which types of severe weather events are most harmful on:
...
#### The csv data file must be kept in the working directory in order for the code to work
```{r DataImport,results="hide"}
library("data.table")
library("ggplot2")
library(reshape2)
library(reshape)
...
path<-getwd()
stormDF <- read.csv(paste0(path,"/repdata_data_StormData.csv"))
...
# Filtering data for injuires or fatalities
...
### 3): Transforming Exponent columns
...
### 5): Total Property and Crop Cost
...
### 6): Total Fatalities and Injuries
...
### 1): Events that had a severe effect on Population Health

Changing format of the table to be put into plot
```{r HealthResults}
bad_stuff <- melt(totalInjuriesDT, id.vars="EVTYPE", variable.name = "bad_thing")
colnames(bad_stuff)[2]<-"bad_thing"
head(bad_stuff, 5)
```
...
### 2): Events that had a severe effect on economy

Changing format to create the plot
```{r EconConsequences}
econ_consequences <- melt(totalCostDT, id.vars="EVTYPE", variable.name = "Damage_Type")
colnames(econ_consequences)[2]<-"Damage_Type"
